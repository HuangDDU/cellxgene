...
export default class TrajectorySVG extends React.PureComponent {
  getTrajectory = (annoMatrix, layoutChoice, trajectoryChoice) => {
    // 从后端读取trajectory数据
  ...
  render() {
    ...
    if (!showTrajectory || !trajectoryChoice.current === "None") return null; // 这里与centroidLabels略有不同

    const { milestonePositionDf, milestonePathDf } = this.getTrajectory(
      annoMatrix,
      layoutChoice,
      trajectoryChoice
    );
